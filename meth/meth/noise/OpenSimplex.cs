...
public class OpenSimplex2S_Exp {
    ...
    public OpenSimplex2S_Exp(long seed, double expBase, double expMin) {
        ...
        perm2 = new short[PSIZE];
        perm2Mult = new double[PSIZE];
        ...
        for (int i = PSIZE - 1; i >= 0; i--) {
            ...
            double expLevel = perm2[i] * 1.0 / PSIZE;
            expLevel = expMin + (1 - expMin) * expLevel;
            expLevel *= 1 - 1 / expBase;
            perm2Mult[i] = -Math.Log(1 - expLevel) / Math.Log(expBase);
            source[r] = source[i];
        }
    }
    ...
    private double noise2_Base(double xs, double ys) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            double mult = perm2Mult[perm2[pxm] ^ pym];
            double extrapolation = grad.dx * dx + grad.dy * dy;

            attn *= attn;
            value += attn * attn * extrapolation * mult;
        }
    ...
    private double noise3_BCC(double xr, double yr, double zr) {
        ...
        while (c != null) {
            ...
            if (attn < 0) {
                ...
                double mult = perm2Mult[perm2[perm2[pxm] ^ pym] ^ pzm];
    ...
    public const double N2 = 0.05481866495625118;
    public const double N3 = 0.2781926117527186;
