...
{
    if (u_maskType == 1) { // Dots
        const vec3 pattern[4] = vec3[4](vec3(1,0,0), vec3(0,1,0), vec3(0,0,1), vec3(0,0,0));
        ivec2 icoords = ivec2(fragcoord);
        return vec4(pattern[(icoords.y * 2 + icoords.x) % 4], 0.25);
    }
    else if (u_maskType == 2) { // Grille
        const vec3 pattern[2] = vec3[2](vec3(0,1,0), vec3(1,0,1));
        return vec4(pattern[int(fragcoord.x) % 2], 0.5);
    }
    else if (u_maskType == 3) { // Wide grille
        const vec3 pattern[4] = vec3[4](
            vec3(1,0,0), vec3(0,1,0), vec3(0,0,1), vec3(0,0,0));
        return vec4(pattern[int(fragcoord.x) % 4], 0.25);
    }
    else if (u_maskType == 4) { // Grille wide soft
        const vec3 pattern[4] = vec3[4](
            vec3(1.0,0.125,0.0),
    ...
    else if (u_maskType == 5) { // Slotmask
        const vec3 pattern[16] = vec3[16](
