...
namespace TrippyGL
{
    ...
    internal sealed class TextureBatchItem : IComparable<TextureBatchItem>
    {
        ...
        /// Calculates and sets all the values in this <see cref="TextureBatchItem"/> except for <see cref="SortValue"/>.
        /// </summary>
        public void SetValue(Texture2D texture, Vector2 position, ref Matrix4x4 worldMatrix, Rectangle? source, Color4b color, Vector2 scale, float rotation, Vector2 origin, float depth)
        {
            Texture = texture;
            Rectangle sourceRect = source ?? new Rectangle(0, 0, (int)texture.Width, (int)texture.Height);
            ...
            float sin = MathF.Sin(rotation);
            float cos = MathF.Cos(rotation);
            VertexTL.Position = Vector3.Transform(new Vector3(cos * tl.X - sin * tl.Y + position.X, sin * tl.X + cos * tl.Y + position.Y, depth), worldMatrix);
            VertexTR.Position = Vector3.Transform(new Vector3(cos * tr.X - sin * tr.Y + position.X, sin * tr.X + cos * tr.Y + position.Y, depth), worldMatrix);
            VertexBL.Position = Vector3.Transform(new Vector3(cos * bl.X - sin * bl.Y + position.X, sin * bl.X + cos * bl.Y + position.Y, depth), worldMatrix);
            VertexBR.Position = Vector3.Transform(new Vector3(cos * br.X - sin * br.Y + position.X, sin * br.X + cos * br.Y + position.Y, depth), worldMatrix);
        ...
        /// without calculating rotation.
        /// </summary>
        public void SetValue(Texture2D texture, Vector2 position, ref Matrix4x4 worldMatrix, Rectangle? source, Color4b color, Vector2 scale, Vector2 origin, float depth)
        {
            Texture = texture;
            Rectangle sourceRect = source ?? new Rectangle(0, 0, (int)texture.Width, (int)texture.Height);
