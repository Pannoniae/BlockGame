...
namespace FontStashSharp
{
	partial class DynamicSpriteFont
	{
			...
			TextStyle textStyle, FontSystemEffect effect, int effectAmount)
		{
            ...
            var text = !source.TextSource.StringText.IsEmpty ? source.TextSource.StringText : source.TextSource.StringBuilderText.ToString().AsSpan();
            if (source.IsNull)
            {
                return 0.0f;
            }
			...
			foreach (var l in lines)
			{
				...
				var shapedText = GetShapedText(line.ToString(), FontSize * FontSystem.FontResolutionFactor);
		...
		private Bounds InternalShapedTextBounds(TextSource source, Vector2 position,
			float characterSpacing, float lineSpacing, FontSystemEffect effect, int effectAmount)
		{
            var text = !source.StringText.IsEmpty ? source.StringText : source.StringBuilderText.ToString().AsSpan();
            if (source.IsNull)
            {
                return Bounds.Empty;
            }
