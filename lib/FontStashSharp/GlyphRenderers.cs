...
namespace FontStashSharp {
    ...
    public static class GlyphRenderers {
        ...
        public static readonly GlyphRenderer Default = (input, output, options) => {
            var bufferSize = options.Size.X * options.Size.Y;
            if (options.Effect == FontSystemEffect.Shadow && options.EffectAmount > 0) {
                // shadow: dilate and make black
                var width = options.Size.X;
                var amt = options.EffectAmount;
                ...
                for (var i = 0; i < bufferSize; ++i) {
                    ...
                    for (int dy = -amt; dy <= amt; dy++) {
                        for (int dx = -amt; dx <= amt; dx++) {
                            var nx = x + dx;
                            var ny = y + dy;
                            if (nx >= 0 && nx < width && ny >= 0 && ny < options.Size.Y) {
                                var ni = ny * width + nx;
                                shadow = Math.Max(shadow, input[ni]);
                            }
                        }
                    }
            ...
            else if (options.Effect == FontSystemEffect.Stroked && options.EffectAmount > 0) {
                var width = options.Size.X;
