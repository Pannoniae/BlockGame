...
/// </summary>
public sealed class TextureBatch : IDisposable {
    ...
    public TextureBatch(GraphicsDevice graphicsDevice, uint initialBatchCapacity = InitialBatchItemsCapacity) {
        if (graphicsDevice == null)
            throw new ArgumentNullException(nameof(graphicsDevice));

        if (initialBatchCapacity <= 0)
            throw new ArgumentOutOfRangeException(nameof(initialBatchCapacity),
                nameof(initialBatchCapacity) + " must be greater than 0.");
        ...
        vertexBuffer =
            new VertexBuffer<VertexColorTexture>(graphicsDevice, InitialBufferCapacity, BufferUsage.StreamDraw);
    }
    ...
    public void SetShaderProgram(SimpleShaderProgram simpleProgram) {
        SetShaderProgram(simpleProgram,
            (simpleProgram != null && simpleProgram.TextureEnabled) ? simpleProgram.Uniforms["samp"] : default);
    }
    ...
    public void SetShaderProgram(ShaderProgram shaderProgram, ShaderUniform textureUniform) {
        if (IsActive)
            throw new InvalidOperationException(nameof(ShaderProgram) + " cant be changed while the " +
                                                nameof(TextureBatcher) + " is active.");
        ...
        if (shaderProgram.GraphicsDevice != GraphicsDevice)
            throw new ArgumentException(nameof(ShaderProgram) + " must belong to the same " + nameof(GraphicsDevice)
                                        + " this " + nameof(TextureBatcher) + " was created with.",
                nameof(shaderProgram));
        ...
        if (!textureUniform.IsEmpty) {
            if (textureUniform.OwnerProgram != shaderProgram)
                throw new ArgumentException(
                    nameof(textureUniform) + " must belong to the provided " + nameof(ShaderProgram) + ".",
                    nameof(textureUniform));

            if (!TrippyUtils.IsUniformSampler2DType(textureUniform.UniformType))
                throw new ArgumentException("The provided " + nameof(ShaderUniform) + " must be a Sampler2D type.",
                    nameof(textureUniform));
        }

        // We check that the ShaderProgram has an attribute in location 0 and is a FloatVec3.
        if (!shaderProgram.TryFindAttributeByLocation(0, out ActiveVertexAttrib attrib) ||
            attrib.AttribType != AttributeType.FloatVec3)
            throw new ArgumentException(
                "The shader program's attribute at location 0 must be of type FloatVec3 (used for position).",
                nameof(shaderProgram));
        ...
        if (shaderProgram.TryFindAttributeByLocation(1, out attrib) && attrib.AttribType != AttributeType.FloatVec4)
            throw new ArgumentException(
                "The shader program's attribute at location 1 must be of type FloatVec4 (used for color).",
                nameof(shaderProgram));
        ...
        if (shaderProgram.TryFindAttributeByLocation(2, out attrib) && attrib.AttribType != AttributeType.FloatVec2)
            throw new ArgumentException(
                "The shader program's attribute at location 2 must be of type FloatVec2 (used for texture coordinates).",
                nameof(shaderProgram));
    ...
    public void Begin(BatcherBeginMode beginMode = BatcherBeginMode.Deferred) {
        ...
        if (ShaderProgram == null)
            throw new InvalidOperationException("A " + nameof(ShaderProgram) + " must be specified (via " +
                                                nameof(SetShaderProgram) + "()) before using Begin().");
    ...
    private bool EnsureBatchListCapacity(int requiredCapacity) {
        ...
        if (currentCapacity < requiredCapacity) {
            ...
            Array.Resize(ref batchItems,
                Math.Min(TrippyMath.GetNextCapacity(currentCapacity, requiredCapacity), (int)MaxBatchItemCapacity));
...
#pragma warning restore CS8774 // Member must have a non-null value when exiting.
        ...
        if (currentCapacity < requiredCapacity)
            vertexBuffer.RecreateStorage(
                Math.Min((uint)TrippyMath.GetNextCapacity((int)currentCapacity, requiredCapacity), MaxBufferCapacity));
    ...
    private TextureBatchItem GetNextBatchItem() {
        ...
        if (!EnsureBatchListCapacity(batchItemCount + 1)) {
            ...
            if (BeginMode == BatcherBeginMode.OnTheFly || BeginMode == BatcherBeginMode.Immediate)
                ...
                throw new InvalidOperationException("Too many " + nameof(TextureBatcher) +
                                                    " items. Try drawing less per Begin()-End() cycle or use OnTheFly or Immediate begin modes.");
        }
    ...
    public void DrawRaw(Texture2D texture, in VertexColorTexture VertexTL, in VertexColorTexture VertexTR,
        ...
        TextureBatchItem item = GetNextBatchItem();
        item.VertexTL = new VertexColorTexture(Vector3.Transform(VertexTL.Position, matrix), VertexTL.Color,
            VertexTL.TexCoords);
        item.VertexTR = new VertexColorTexture(Vector3.Transform(VertexTR.Position, matrix), VertexTR.Color,
            VertexTR.TexCoords);
        item.VertexBR = new VertexColorTexture(Vector3.Transform(VertexBR.Position, matrix), VertexBR.Color,
            VertexBR.TexCoords);
        item.VertexBL = new VertexColorTexture(Vector3.Transform(VertexBL.Position, matrix), VertexBL.Color,
            VertexBL.TexCoords);
        item.Texture = texture;
    ...
    public void Draw(Texture2D texture, in Matrix3x2 transform, Rectangle? source, Color4b color, float depth = 0) {
        ...
        TextureBatchItem item = GetNextBatchItem();
        item.SetValue(texture, transform, source ?? new Rectangle(0, 0, (int)texture.Width, (int)texture.Height), color,
            depth);
    ...
    /// <param name="depth">The depth at which to draw the texture.</param>
    public void Draw(Texture2D texture, in Matrix3x2 transform, Rectangle? source, Color4b color, Vector2 origin,
        float depth = 0) {
        StartDraw(texture);
    ...
    public void Draw(Texture2D texture, Vector2 position, Rectangle? source = null, float depth = 0) {
        ...
        TextureBatchItem item = GetNextBatchItem();
        item.SetValue(texture, position, source ?? new Rectangle(0, 0, (int)texture.Width, (int)texture.Height),
            Color4b.White, depth);
    ...
    public void Draw(Texture2D texture, RectangleF destination, Rectangle? source, Color4b color, float depth = 0) {
        ...
        TextureBatchItem item = GetNextBatchItem();
        item.SetValue(texture, destination, source ?? new Rectangle(0, 0, (int)texture.Width, (int)texture.Height),
            color, depth);
    ...
    public void DrawString(TextureFont font, ReadOnlySpan<char> text, Vector2 position, Color4b color, Vector2 scale,
        ...
        for (int i = 0; i < text.Length; i++) {
            ...
            if (source.Width != 0) {
                ...
                renderOffset = new Vector2(cos * renderOffset.X - sin * renderOffset.Y,
                    sin * renderOffset.X + cos * renderOffset.Y);
                batchItem.SetValue(font.Texture, penPosition + kroff + renderOffset, source, color, scale, sin, cos,
                    depth);
    ...
    /// <param name="origin">The origin for rotation and scaling in pixel coordinates.</param>
    /// <param name="depth">The depth at which to draw the string of text.</param>
    public void DrawString(TextureFont font, ReadOnlySpan<char> text, Vector2 position, Color4b color, float scale,
        float rotation, Vector2 origin, float depth = 0) {
    ...
    public void DrawString(TextureFont font, ReadOnlySpan<char> text, Vector2 position, Color4b color, Vector2 scale,
        ...
        for (int i = 0; i < text.Length; i++) {
            ...
            if (source.Width != 0) {
                TextureBatchItem batchItem = GetNextBatchItem();
                batchItem.SetValue(font.Texture, new Vector2(x, y + koff.Y) + font.GetRenderOffset(c) * scale, source,
                    color, scale, depth);
                SetItemSortKey(batchItem);
            }
    ...
    public void DrawString(TextureFont font, ReadOnlySpan<char> text, Vector2 position, Color4b color,
        ...
        for (int i = 0; i < text.Length; i++) {
            ...
            if (source.Width != 0) {
                TextureBatchItem batchItem = GetNextBatchItem();
                batchItem.SetValue(font.Texture, new Vector2(x, y + koff.Y) + font.GetRenderOffset(c), source, color,
                    depth);
                SetItemSortKey(batchItem);
            }
    ...
    /// <param name="origin">The origin for rotation and scaling in pixel coordinates.</param>
    /// <param name="depth">The depth at which to draw the string of text.</param>
    public void DrawString(TextureFont font, StringBuilder text, Vector2 position, Color4b color, Vector2 scale,
        float rotation, Vector2 origin, float depth = 0) {
        if (font == null)
            throw new ArgumentNullException(nameof(font));
    ...
    /// <param name="origin">The origin for rotation and scaling in pixel coordinates.</param>
    /// <param name="depth">The depth at which to draw the string of text.</param>
    public void DrawString(TextureFont font, StringBuilder text, Vector2 position, Color4b color, float scale,
        float rotation, Vector2 origin, float depth = 0) {
        DrawString(font, text, position, color, new Vector2(scale, scale), rotation, origin, depth);
    }
    ...
    private void Flush(bool sameTextureEnsured) {
        ...
        while (itemStartIndex < batchItemCount) {
            ...
            int itemEndIndex = sameTextureEnsured
                ? batchItemCount
