...
// (note: it's at https://github.com/ppy/osu/)
...
public static partial class NVAPI {
    ...
    public const string APPLICATION_NAME = @"BlockGame";
    public const string PROFILE_NAME = @"BlockGame";
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    public delegate NvStatus FindApplicationByNameDelegate(IntPtr sessionHandle,
        [MarshalAs(UnmanagedType.BStr)] string appName, out IntPtr profileHandle, ref NvApplication application);
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    public delegate NvStatus GetSettingDelegate(IntPtr sessionHandle, IntPtr profileHandle, NvSettingID settingID,
        ref NvSetting setting);

    public static GetSettingDelegate GetSetting;

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate NvStatus CreateProfileDelegate(IntPtr sessionHandle, ref NvProfile profile,
        out IntPtr profileHandle);

    private static readonly CreateProfileDelegate CreateProfile;

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate NvStatus FindProfileByNameDelegate(IntPtr sessionHandle,
        [MarshalAs(UnmanagedType.BStr)] string profileName, out IntPtr profileHandle);

    private static readonly FindProfileByNameDelegate FindProfileByName;
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate NvStatus EnumApplicationsDelegate(IntPtr sessionHandle, IntPtr profileHandle, uint startIndex,
        ref uint appCount, [In, Out, MarshalAs(UnmanagedType.LPArray)] NvApplication[] applications);

    private static readonly EnumApplicationsDelegate EnumApplications;

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate NvStatus CreateApplicationDelegate(IntPtr sessionHandle, IntPtr profileHandle,
    ...
    public static bool UseDedicatedGpu {
        set {
            if (!Available)
                return;
    ...
    private static bool containsApplication(IntPtr profileHandle, NvProfile profile, out NvApplication application) {
        ...
        for (uint i = 0; i < numApps; i++) {
            if (applications[i].AppName == filename) {
    ...
    public static bool getProfile(out IntPtr profileHandle, out NvApplication application,
        ...
        if (checkError(FindApplicationByName(sessionHandle, filename, out profileHandle, ref application),
    ...
    /// <returns>If the operation succeeded.</returns>
    public static bool getSetting(NvSettingID settingId, IntPtr profileHandle, out NvSetting setting) {
    ...
    static NVAPI() {
        ...
        try {
            ...
            if (initialize?.Invoke() == NvStatus.OK) {
                ...
                getDelegate(0x7E4A9A0B, out FindProfileByName);
...
public struct NvSetting {
    ...
    public static uint Stride => (uint)Marshal.SizeOf<NvSetting>() | (1 << 16);
}
...
public enum NvStatus {
    ...
    IMPLICIT_SET_GPU_TOPOLOGY_CHANGE_NOT_ALLOWED =
        -128, // There is no implicit GPU topology active. Use SetHybridMode to change topology.
    ...
    DISPLAYCONFIG_VALIDATION_FAILED =
    ...
    GPU_WORKSTATION_FEATURE_INCOMPLETE =
