...
public partial class OverworldWorldGenerator {
    ...
    public void generate(ChunkCoord coord) {
        ...
        for (int x = 0; x < Chunk.CHUNKSIZE; x++) {
            for (int z = 0; z < Chunk.CHUNKSIZE; z++) {
                ...
                chunk.setBlockFast(x, 0, z,  Block.HELLROCK.id);
                // hack until we can propagate them properly AND cheaply
                chunk.setBlockLightDumb(x, 0, z, Block.lightLevel[Block.HELLROCK.id]);
    ...
    public void placeMapleTree(int x, int y, int z) {
        for (int i = 0; i < 7; i++) {
            world.setBlockDumb(x, y + i, z,  Block.MAPLE_LOG.id);
        }
        ...
        for (int x2 = -1; x2 <= 1; x2++) {
            ...
            world.setBlockDumb(x + x2, y + 4, z,  Block.MAPLE_LEAVES.id);
        }
        for (int z2 = -1; z2 <= 1; z2++) {
            ...
            world.setBlockDumb(x, y + 4, z + z2,  Block.MAPLE_LEAVES.id);
        }
        // leaves, thick
        for (int x2 = -1; x2 <= 1; x2++) {
            for (int z2 = -1; z2 <= 1; z2++) {
                ...
                for (int y2 = 5; y2 <= 6; y2++) {
                    world.setBlockDumb(x + x2, y + y2, z + z2,  Block.MAPLE_LEAVES.id);
                }
            }
        }
        // leaves, thin on top
        for (int x2 = -1; x2 <= 1; x2++) {
            world.setBlockDumb(x + x2, y + 7, z,  Block.MAPLE_LEAVES.id);
        }
        for (int z2 = -1; z2 <= 1; z2++) {
            world.setBlockDumb(x, y + 7, z + z2,  Block.MAPLE_LEAVES.id);
            world.setBlockDumb(x, y + 8, z,  Block.MAPLE_LEAVES.id);
        }
    }
    ...
    public void placeJungleTree(int x, int y, int z) {
        for (int i = 0; i <= 6; i++) {
            if (i == 0) {
                for (int x3 = -1; x3 <= 1; x3++) {
                    world.setBlockDumb(x + x3, y, z,  Block.MAPLE_LOG.id);
                }
                for (int z3 = -1; z3 <= 1; z3++) {
                    world.setBlockDumb(x, y, z + z3,  Block.MAPLE_LOG.id);
                }
            }
            world.setBlockDumb(x, y + i, z,  Block.MAPLE_LOG.id);
        }

        world.setBlockDumb(x + 1, y + 4, z,  Block.MAPLE_LOG.id);
        world.setBlockDumb(x - 1, y + 6, z,  Block.MAPLE_LOG.id);
        world.setBlockDumb(x - 1, y + 7, z,  Block.MAPLE_LOG.id);
        world.setBlockDumb(x - 1, y + 8, z,  Block.MAPLE_LOG.id);

        for (int x3 = 1; x3 <= 2; x3++) {
            for (int z3 = -1; z3 <= 1; z3++) {
                world.setBlockDumb(x + x3, y + 5, z + z3,  Block.MAPLE_LEAVES.id);
            }
        }

        world.setBlockDumb(x + 3, y + 5, z,  Block.MAPLE_LEAVES.id);

        for (int x3 = 1; x3 <= 2; x3++) {
            world.setBlockDumb(x + x3, y + 6, z,  Block.MAPLE_LEAVES.id);
        }
        ...
        for (int x3 = -3; x3 <= 2; x3++) {
            for (int z3 = -1; z3 <= 1; z3++) {
                ...
                world.setBlockDumb(x + x3, y + 8, z + z3,  Block.MAPLE_LEAVES.id);
            }
        }

        for (int x3 = -2; x3 <= 1; x3++) {
            world.setBlockDumb(x + x3, y + 9, z,  Block.MAPLE_LEAVES.id);
        }
        world.setBlockDumb(x - 1, y + 5, z,  Block.MAPLE_LEAVES.id);
    }
}